...
router.post("/", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    // Convert parentId to ObjectId or null
   // Convert parentId to ObjectId or null
const parent = parentId ? new mongoose.Types.ObjectId(parentId) : null;

// Check if already exists
const exists = await Master.findOne({ name, parent });
...
await master.save();
    ...
    // Auto-seed Status children if root
    if (!parent && name.trim().toLowerCase() === "status") {
